...
namespace
{
	...
	{
		Pkg = CreatePackage(*FString::Printf(TEXT("/Game/Materials/%s"), Name)); Pkg->FullyLoad();
		return NewObject<UMaterial>(Pkg, Name, RF_Public | RF_Standalone);
	}
	...
	UMaterialExpression* Custom3(UMaterial* M, int32 Y, float Def) { auto* CD3 = Expr<UMaterialExpressionPerInstanceCustomData>(M, -700, Y); CD3->DataIndex = 3; CD3->ConstDefaultValue = Def; return CD3; }

	bool MakeNeon()
	{
		UPackage* Pkg; UMaterial* M = NewMat(TEXT("M_Neon"), Pkg); UMaterialEditorOnlyData* ED = M->GetEditorOnlyData();
		...
		ED->BaseColor.Connect(0, Color); ED->EmissiveColor.Connect(0, Out); ED->Metallic.Connect(0, Met); ED->Roughness.Connect(0, Rgh);
		return SaveMat(Pkg, M);
	}
	bool MakeNeonInst()
	{
		UPackage* Pkg; UMaterial* M = NewMat(TEXT("M_NeonInst"), Pkg); UMaterialEditorOnlyData* ED = M->GetEditorOnlyData();
		...
		UMaterialExpression* RGB = CustomRGB(M, 0); UMaterialExpression* CD3 = Custom3(M, 240, 1.f);
		...
		ED->BaseColor.Connect(0, Base); ED->EmissiveColor.Connect(0, Out); ED->Metallic.Connect(0, Met); ED->Roughness.Connect(0, Rgh);
		return SaveMat(Pkg, M);
	}
	bool MakeGlowInst()
	{
		...
		ED->BaseColor.Connect(0, Base); ED->EmissiveColor.Connect(0, Out); ED->Roughness.Connect(0, Rgh);
		return SaveMat(Pkg, M);
	}
	bool MakeHolo()
	{
		...
		UMaterialExpression* RGB = CustomRGB(M, 0); UMaterialExpression* CD3 = Custom3(M, 240, 0.3f);
	...
	{
		UPackage* Pkg = CreatePackage(*FString::Printf(TEXT("/Game/Meshes/%s"), Name)); Pkg->FullyLoad();
...
{
	bool Ok = MakeNeon() && MakeNeonInst() && MakeGlowInst() && MakeHolo();
